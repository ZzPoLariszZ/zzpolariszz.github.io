...
- \\(\mathcal{A}\\) receives \\( c_b = (g^{x_2}, h^{x_2} \cdots m_b) \\) from \\(\mathcal{B}\\). Here takes \\(x_2\\) as randomness \\(r \in \mathbb{Z}_q\\).

- According to the assumption of \\(\mathcal{A}\\)'s capability,  

    \\(\mathcal{A}\\) can return the correct \\(b^\prime = b\\),

    which means \\(\mathcal{B}\\) can distinguish between \\(c_b\\) and \\(c_{b^\prime}\\),

    which means \\(\mathcal{B}\\) can distinguish between \\(c_2 = h^{x_2} \cdots m_b\\) and \\(c_2 = g^y \cdots m_b\\),
