...
Given an integer \\(n = p \cdot q\\) that is the product of two unknown primes \\(p\\) and \\(q\\),  
...
<u>Slight Improvement</u>: \\(O(2^{\lambda} \cdot t)\\)  

- since \\(p\\) and \\(q\\) have the same length \\(\lambda\\), the searched value can be limited \\(x \in [2, \dots, \sqrt n]\\); otherwise, the \\(p \cdot q > n\\).
