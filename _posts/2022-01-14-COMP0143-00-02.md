...
$$
m \equiv c^d \equiv {(m^e)}^d \equiv m^{ed} \equiv m^{1+k\phi(n)} \equiv m({m^{\phi(n)}}^k) \equiv m(1^k) \equiv m \, (\text{mod} \, n)
$$
...
- If \\(M \in \mathbb{Z}_n \setminus \mathbb{Z}_n^{\*}\\), which means \\(gcd(m, n) \neq 1\\)
    ...
    - \\(gcd(M, n) = p \implies gcd(M, q) = 1 \implies M \in \mathbb{Z}_q^{\*}\\)
        ...
        $$
        M \, (\text{mod} \, n) \mapsto (M \, (\text{mod} \, p), \ M \, (\text{mod} \, q))
        $$
        ...
        $$
        M \equiv c^d \equiv {(M^e)}^d \equiv M^{ed} \equiv M^{1+k(p-1)(q-1)} \equiv M^{1+k^\prime(q-1)} \equiv M({m^{q-1}}^{k^\prime}) \equiv M(1^{k^\prime}) \equiv M \, (\text{mod} \, q)
        $$

    - \\(gcd(M, n) = q \implies gcd(M, p) = 1 \implies M \in \mathbb{Z}_p^{\*}\\)
        ...
        $$
        M \equiv c^d \equiv {(M^e)}^d \equiv M^{ed} \equiv m^{1+k(p-1)(q-1)} \equiv m^{1+k^\prime(p-1)} \equiv m({m^{p-1}}^{k^\prime}) \equiv m(1^{k^\prime}) \equiv m \, (\text{mod} \, n)
        $$
