...
- \\(Adv_{\mathcal{B}, \text{Gen}}^{\text{DDH}}(\lambda) = | Pr[\text{Game}^{\text{IND-CPA}}(\lambda) = 1] - \frac{1}{2} | = Adv_{\mathcal{A}, \text{ElGamal}}^{\text{IND-CPA}}(\lambda)\\) by the definition of DDH Problem.

- \\(Adv_{\mathcal{A}, \text{ElGamal}}^{\text{IND-CPA}}(\lambda) = Adv_{\mathcal{B}, \text{Gen}}^{\text{DDH}}(\lambda) \leq negl(\lambda)\\) such that ElGamal Encryption has IND-CPA Security when DDH Problem is computationally hard.
