...
UCL Course COMP0133 LEC-07: Paxos
...
**Reason:** More likely to keep liveness without sacrificing safety (work through three phases & not keep waiting for a leader)
...
\\( \implies \\) The proposer of 11 must receive its \\( R_1() \\) from at least one node that saw \\( Q_2(10, v_{10}) \\) because of overlap in majority
...
- If this node did restart, it must remember \\( v_a \\) and \\( n_a \\) on disk before sending \\( R_2() \\)
    ...
    since the agreed-on value \\( v_a \\) has already be set in some node by \\( Q_3() \\) and known by receiving \\( R_1() \\) from majority of the old leader
