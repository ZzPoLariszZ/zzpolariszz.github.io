...
> Property: \\(\mathbb{Z}_p^{\*}\\) must have a cyclic subgroup of prime order \\(q\\) - \\(\mathbb{Z}_q^{\*}\\)
...
> The order of \\(\mathbb{Z}_p^{\*}\\): \\(\|\mathbb{Z}_p^{\*}\| = \phi(p) = p - 1 = 2q\\).  
> \\(\mathbb{Z}_p^{\*}\\) must have a non-trivial subgroup of order \\(2\\) and a non-trivial subgroup of prime order \\(q\\) (via Lagrange's Theorem).  
> The subgroup of order \\(2\\) has two elements: \\(\\{\, 1, p - 1 \, \\}\\).  
> \\(g := h^2 \ mod \ p \ \\) where \\( \ h \xleftarrow{\$} \mathbb{Z}_p^{\*} \setminus \\{\, 1, p - 1\,\\}\\)  
> will always let \\(g\\) be an element in a non-trivial subgroup of prime order \\(q\\) and not the identity element \\(e\\).  
> Since the subgroup of prime order \\(q\\) is also a cycle group, and the number of generators inside is \\(\phi(q) = q - 1\\) (via Euler’s Totient function).  
> Therefore, all non-identity elements can generate the subgroup of prime order \\(q\\) (\\(g\\) and \\(h\\) can be randomly selected).

Let \\(M \in \\{0,1\\}^{l-1}\\) be a binary string of the message with length \\(l - 1\\).

Consider \\(M\\) as an integer \\(M \in [0, q-1]\\). 

**Encoding**

1. Compute \\(y = M + 1 \in [1, q]\\).
