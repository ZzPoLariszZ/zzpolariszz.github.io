...
According to the assumption of \\(\mathcal{B}\\)'s capability,  

\\(\mathcal{B}\\) can get \\(p\\) and \\(q\\),  
...
which means that \\(\mathcal{A}\\) can output an output \\(m\\) that meets \\( m^e \equiv c (\text{mod} \, n)\\) to break RSA.
