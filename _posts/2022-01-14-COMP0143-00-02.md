...
3. \\(x_1, x_2 \xleftarrow{\$} \mathbb{Z}_q\\): Challenger picks two random integers \\(x_1, x_2\\).

4. \\(y_1 = g^{x_1}, y_2 = g^{x_2}\\): Challenger sends \\(y_1, y_2\\) to Adversary.

5. \\(h \leftarrow \mathcal{A}(\mathbb{G}, g, q, y_1, y_2)\\): Adversary outputs his result and sends back to Challenger.

6.  \\(\text{return} \ h = g^{x_1 x_2}\\): If the output from Adversary meets the requirement, return \\(1\\); otherwise, return \\(0\\).
