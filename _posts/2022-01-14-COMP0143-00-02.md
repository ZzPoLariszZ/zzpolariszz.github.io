...
- The subgroup of order \\(2\\) has two elements: \\(\\{\, 1, p - 1 \, \\}\\).  
    ...
    Since the subgroup of prime order \\(q\\) is also a cycle group, 

    and the number of generators inside is \\(\phi(q) = q - 1\\) (via Euler’s Totient Function).
    
    Therefore, all non-identity elements can generate the subgroup of prime order \\(q\\) 
    
    such that \\(g\\) and \\(h\\) can be randomly selected.
