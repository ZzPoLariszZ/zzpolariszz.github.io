...
- \\(\phi(n)\\) is the order of \\(\mathbb{Z}_n^*\\) since it counts the number of integers less than \\(n\\)  that are co-prime to \\(n\\). 
...
$$
m \equiv c^d \equiv (m^e)^d \equiv m^{ed} \equiv m (\text{mod} \, n)
$$
