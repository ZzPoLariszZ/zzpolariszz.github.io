...
A Public Key Encryption (PKE) scheme de ned over \\((\mathcal{K}, \mathcal{M}, \mathcal{C})\\) is a tuple of efficient algorithms \\((\text{Gen}, \text{Enc}, \text{Dec})\\):
...
- \\(\text{Dec}(sk, c)\\): Decrypt the ciphertext \\(m\\) using \\(sk\\) and output the plaintext \\(c\\).
