...
#### Application in Bitcoin: SHA-256
...
- Rate Size \\(r\\): the portion of the state that interacts with the input/output (e.g., 1088 bits for Keccak-256),
...
- If more output is needed (e.g., length \\(l\\)), take the first \\(r\\) bits of the state,  
    and apply Keccak-\\(f[1600]\\) permutation function again, take the first \\(l-r\\) bits of the state,  
    and then concatenate two extracted bits together to get the final hash output
