...
**<span style="color:Red">There is currently NO known algorithm (except for Quantum Algorithm) <br> that can get \\(x\\) in polynomial time if the order \\(q\\) is a large prime (computationally hard).</span>**
