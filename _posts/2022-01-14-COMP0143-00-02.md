...
Given a cyclic group \\(\mathbb{G} = <g>\\) of order \\(\|\mathbb{G}\| = q\\) with generator \\(g\\), and some element \\(h \in \mathbb{G}\\),  
the Discrete Logarithm Problem (DLP) is defined as the problem of finding \\(x \in \mathbb{Z}_n\\) such that \\(h = g^x\\).
