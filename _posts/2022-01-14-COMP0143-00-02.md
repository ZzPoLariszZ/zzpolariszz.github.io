...
> \\(p\\) is Safe Prime and \\(q\\) is Sophie Germain Prime [(Wiki - Safe and Sophie Germian Primes)](https://en.wikipedia.org/wiki/Safe_and_Sophie_Germain_primes)
...
such that \\(g\\) is one generator of a subgroup of prime order \\(q\\) in \\(\mathbb{Z}_p^{\*}\\).
