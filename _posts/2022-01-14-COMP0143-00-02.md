...
- Compute the \\(d\\) which is the inverse of \\(e\\) \\(\implies ed \equiv 1 (\text{mod} \, \phi(n))\\).
...
$$
m \equiv c^d \equiv {(m^e)}^d \equiv m^{ed} \equiv m^{1+h\phi(n)} \equiv m({m^{\phi(n)}}^h) \equiv m(1^h) \equiv m (\text{mod} \, n)
$$

**Proof for Hardness**

$$
Pr[\text{Game}_{\mathcal{A}, \text{Gen}_{\text{RSA}}}^{\text{RSA}}(\lambda) = 1] \leq negl(\lambda)
$$
...
2. \\(\Big((e, n), (d, n) \Big) \xleftarrow{\$} \text{Gen}_{\text{RSA}}\\): Challenger generates \\(\Big((e, n), (d, n)\Big)\\) and keeps \\((d, n)\\), then sends \\((e, n)\\) to Adversary.
...
Let \\(\mathcal{B}\\)  be a PPT adversary against Integer Factorization Problem that simulates RSA Cryptosystem for \\(\mathcal{A}\\).

1. \\(\mathcal{A}\\) requests a new random ciphertext from \\(\mathcal{B}\\)  

**Not Have IND-CPA Security**

**<span style="color:Red">Text-book RSA does not have IND-CPA security since it is deterministic.</span>**
