...
- \\(\theta\\) Step: computes the parity of each column and mixes this parity into the state (in-slice diffusion)

- \\(\rho\\) Step: applies a cyclic shift (rotation) to each lane to spread bits over the lanes (inter-slice diffusion)

- \\(\pi\\) Step: rearranges the positions of the lanes (complete diffusion)

- \\(\Chi\\) Step: combines bits using a non-linear function (non-linearity)

    $$
    \forall \, x \in \{0, 1, 2, 3, 4\}, \forall \, y \in \{0, 1, 2, 3, 4\}, \quad S[x][y] = S[x][y] \oplus \Big( \,(\neg \, S[x+1][y]) \land S[x+2][y] \, \Big)
    $$

- \\(\lota\\) Step: adds a round constant to the state (break symmetries)
