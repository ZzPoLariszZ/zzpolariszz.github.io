...
UCL Course COMP0133 LEC-12: Cryptographic Primitives
...
- When \\( a \\) and \\( n \\) are relatively prime (coprime), modualr inverse \\( x \\) exists and is unique (can be found by *<span style="color:Red">Extended Euclidean Algorithm</span>*)
...
1. Choose two large prime numbers \\( p \\) and \\( q \\) where \\( |p| = |q| \\)
