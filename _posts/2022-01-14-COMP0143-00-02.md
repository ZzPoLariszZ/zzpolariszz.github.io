...
1. Compute \\(y = M + 1 \in \\{1, 2, \dots, q\\}\\).
    ...
    - Find that \\(y \in \mathbb{Z}_p^{\*} = \\{1, 2, \dots, p-1\\} \\) since \\(p - 1 = 2q\\).
...
2. Compute \\(z = y^2 \ mod \ p\\) which enures that \\(z \in \mathbb{G}\\).
    ...
    - Since squaring is a two-to-one mapping in \\(\mathbb{Z}_p^{\*}\\),  
        ...
        the order of subgroup formed by quadratic residues is half the order of\ \(\mathbb{Z}_p^{\*}\\),

        which is \\(\frac{1}{2} \cdot \phi(p) = \frac{1}{2} \cdot 2q = q\\)
