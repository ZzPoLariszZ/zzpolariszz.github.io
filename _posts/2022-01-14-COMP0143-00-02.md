...
- \\(Enc_{\{e,n\}}(m): c \leftarrow (r \ || \ m)^e \ mod \ n\\) where \\(r \xleftarrow{\$} \{0,1\}^{\|n\| - l(\lambda) - 1}\\)
...
e.g., \\(l(\lambda) \geq 2\lambda - O(log\lambda)\\), the required trials for a CPA attack is \\(2^{O(log\lambda)}\\),  
