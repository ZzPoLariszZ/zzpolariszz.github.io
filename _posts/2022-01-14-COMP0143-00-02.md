...
- The square operation in \\(g := h^2 \ mod \ p \ \\) where \\( \ h \xleftarrow{\$} \mathbb{Z}_p^{\*} \setminus \\{\, 1, p - 1\,\\}\\)  
    ...
    Moreover, since the subgroup of prime order \\(q\\) is also a cycle group, 
    ...
    such that \\(g\\) and \\(h\\) can be randomly selected from their scope.

Let \\(M \in \\{0,1\\}^{l-1}\\) be a binary string of the message with length \\(l - 1\\).

Consider \\(M\\) as an integer \\(M \in [0, q-1]\\): \\(M \in \mathbb{Z}_q\\). 
