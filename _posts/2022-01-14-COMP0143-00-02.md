...
> \\(\mathbb{Z}_p^{\*}\\) must have a subgroup of order \\(2\\) and a subgroup of order \\(q\\) (via Lagrange's Theorem).  
> The subgroup of order \\(2\\) has two elements: \\(\\{\, 1, p - 1 \, \\}\\).  
> \\(g := h^2 \ mod \ p \ \\) where \\( \ h \xleftarrow{\$} \mathbb{Z}_p^{\*} \setminus \\{\, 1, p - 1\,\\}\\) will always let \\(g\\) be element in a subgroup of prime order \\(q\\).
