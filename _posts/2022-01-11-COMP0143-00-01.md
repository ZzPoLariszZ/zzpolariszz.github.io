...
$$
\textit{key} := \text{Keccak-256}\Big(\text{RLP}(\textit{value})\Big)
$$
...
def rlp_encode(input):
    if isinstance(input,str):
        ...
        if len(input) == 1 and ord(input) < 0x80:
            # Single-byte String (0x00 to 0x7f)
            return input
        # Longer String or Bytes >= 0x80
...
def encode_length(L, offset):
    ...
    raise Exception("input too long")  # Length Too Long
...
Short for *Practical Algorithm To Retrieve Information Coded in Alphanumeric*.
