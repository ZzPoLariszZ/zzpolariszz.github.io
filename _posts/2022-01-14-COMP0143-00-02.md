...
<u>Slight Improvement</u>: \\(O(2^{\lambda} \cdot t)\\), since

- \\(p\\) and \\(q\\) have the same length \\(\lambda\\), the searched value can be limited \\(x \in [2, \dots, \sqrt n]\\); otherwise, the \\(pq > n\\).
...
- Pick a random integer \\(e \in [1, \dots, \phi(n)-1]\\) which is co-prime with \\(phi(n)\\) \\(\implies gcd(e, \phi(n)) = 1\\).
