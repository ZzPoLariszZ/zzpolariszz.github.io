...
- \\(\mathcal{A}\\) receives public key \\(y = g^{x_1}\\) from \\(\mathcal{B}\\).

- \\(\mathcal{A}\\) submits two distinct chosen plaintexts \\((m_0, m_1)\\) to \\(\mathcal{B}\\).

- \\(\mathcal{A}\\) receives \\( c_b = (g^{x_2}, y^{x_2} \cdots m_b) \\) from \\(\mathcal{B}\\). Here takes \\(x_2\\) as randomness \\(r \in \mathbb{Z}_q\\).
